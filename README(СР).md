...
- `def gcd(a, b):`  
  Объявляет функцию `gcd` с двумя параметрами — числами `a` и `b`.

- `if b == 0:`  
  Проверяет условие завершения рекурсии: если второе число равно нулю, алгоритм останавливается.

- `return a`  
  Возвращает первое число как результат, так как НОД(a, 0) = |a| (в текущей реализации предполагаются неотрицательные числа).

- `return gcd(b, a % b)`  
  Выполняет рекурсивный вызов с новыми аргументами: второе число становится первым, а остаток от деления `a % b` — вторым. Это сокращает задачу до эквивалентной с меньшими значениями.
